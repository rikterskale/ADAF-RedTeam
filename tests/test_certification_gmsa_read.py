"""Lab-gated certification test for gmsa-read-authorization.

Reads msDS-GroupMSAMembership only — never msDS-ManagedPassword.
...
Required env vars when running:
  ...
  ADAF_RT_LAB_GMSA_DN            gMSA object DN
...
def _engagement(tmp_path) -> str:
    ...
    eng = {
        ...
        "capabilities": {"gmsa-read-authorization": {
...
def test_gmsa_read_certification_lab_run(tmp_path, capsys):
    ...
    rc = main([
        ...
        "--capability", "gmsa-read-authorization",
        "--source-address", os.environ["ADAF_RT_LAB_SOURCE_ADDR"],
        "--target", target,
        "--domain", os.environ["ADAF_RT_LAB_DOMAIN"],
        "--finding-id", "F-CERT-GMSA000000",
        "--control-id", "ADAF-CERT-GMSA",
        "--out", str(out),
    ])
    ...
    for bad in ("managedpassword", "msds-managedpassword", "-----begin", "nthash"):
